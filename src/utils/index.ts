...
import { ONLINE_API_BASE } from '@/constants/api'
 ...
 *
 * - trial（体验版）、release（正式版）：固定走线上 ONLINE_API_BASE
 * - develop（开发版）：dev 构建默认本地；production 构建走线上
 */
export function getEnvBaseUrl() {
  let baseUrl = import.meta.env.VITE_SERVER_BASEURL || ONLINE_API_BASE
  ...
  // 体验版、正式版：始终线上（不受 development 本地配置影响）
  if (envVersion === 'trial' || envVersion === 'release') {
    return (
      import.meta.env.VITE_SERVER_BASEURL__WEIXIN_TRIAL
      || import.meta.env.VITE_SERVER_BASEURL__WEIXIN_RELEASE
      || ONLINE_API_BASE
    )
  }

  // 开发版 develop
  const developUrl = import.meta.env.VITE_SERVER_BASEURL__WEIXIN_DEVELOP
  if (developUrl) {
    return developUrl
  }

  // production 构建的开发版：回退线上
